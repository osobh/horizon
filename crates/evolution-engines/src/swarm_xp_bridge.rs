...
use crate::swarm::{SwarmConfig, SwarmEngine};
...
impl SwarmXPFitnessFunction {
    ...
    fn evaluate_social_learning_fitness(&self, agent: &Agent) -> f64 {
        ...
        let social_xp_count = stats
            .xp_history
            .iter()
            .filter(|entry| {
                entry.category.contains("social")
    ...
    fn evaluate_exploration_balance_fitness(
        &self,
        agent: &Agent,
        evolvable_agent: Option<&EvolvableAgent>,
    ) -> f64 {
        let stats = futures::executor::block_on(agent.stats());
        ...
        let performance_stability = if stats.goals_processed >= 8 {
            let recent_success_rate = if stats.goals_processed >= 16 {
                let recent_successes = stats
                    .goals_succeeded
                    .saturating_sub(stats.goals_processed / 2);
...
impl XPFitnessFunction for SwarmXPFitnessFunction {
    ...
    fn calculate_xp_reward(
        ...
        let balance_bonus = if evolution_metrics.processing_speed > 1.0
            && evolution_metrics.processing_speed < 2.0
        {
            40u64 // Good balance
        } else {
            0u64
    ...
    async fn should_evolve(&self, agent: &Agent) -> bool {
        ...
        if stats.current_xp >= 110 {
            // Moderate threshold for swarm agents
            return true;
        }

        // Check for sufficient swarm interactions
        let interaction_count = stats
            .xp_history
            .iter()
            .filter(|entry| {
                entry.category.contains("social")
...
impl SwarmXPEngine {
    ...
    pub async fn initialize_swarm_xp_population(
        &self,
        size: usize,
    ) -> EvolutionEngineResult<Vec<Agent>> {
        ...
        for individual in &population.individuals {
            ...
            agent
                .award_xp(
                    initial_xp,
                    "Initial swarm participation potential".to_string(),
                    "swarm_initialization".to_string(),
                )
                .await
                .map_err(|e| EvolutionEngineError::InitializationError {
    ...
    fn calculate_swarm_potential_xp(&self, evolvable_agent: &EvolvableAgent) -> u64 {
        ...
        let risk_balance_bonus =
            ((1.0 - (genome.behavior.risk_tolerance - 0.4).abs() / 0.6) * 50.0) as u64;
    ...
    pub async fn evolve_with_swarm_xp(
        &mut self,
    ) -> EvolutionEngineResult<(Vec<EvolutionResult>, XPEvolutionStats, SwarmXPStats)> {
    ...
    async fn get_swarm_xp_stats(&self) -> EvolutionEngineResult<SwarmXPStats> {
        ...
        for agent in agent_population.iter() {
            ...
            social_learning_events += stats
                .xp_history
                .iter()
                .filter(|entry| {
                    entry.category.contains("social") || entry.category.contains("swarm")
                })
                .count() as u64;
        ...
        let best_global_fitness = convergence_history
            .iter()
        ...
        for agent in agent_population.iter() {
            agent
                .award_xp(
                    total_xp,
                    format!(
                        "Swarm milestone: {} (quality: {:.2})",
                        milestone_type, convergence_quality
                    ),
                    "swarm_convergence".to_string(),
                )
                .await
...
mod tests {
    ...
    async fn test_swarm_xp_fitness_function() {
        ...
        agent
            .award_xp(
                80,
                "Social learning test".to_string(),
                "social_learning".to_string(),
            )
            .await
            .unwrap();
