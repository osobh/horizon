...
use super::improvement::{GrowthHistory, GrowthPattern};
...
impl DgmXPFitnessFunction {
    ...
    fn evaluate_pattern_discovery_fitness(
        &self,
        _agent: &Agent,
        growth_history: Option<&GrowthHistory>,
    ) -> f64 {
    ...
    fn evaluate_growth_momentum_fitness(&self, agent: &Agent) -> f64 {
        ...
        if stats.xp_history.len() >= 5 {
            let recent_gains: Vec<u64> = stats
                .xp_history
                .iter()
                .take(5)
                .map(|entry| entry.amount)
...
impl XPFitnessFunction for DgmXPFitnessFunction {
    ...
    async fn should_evolve(&self, agent: &Agent) -> bool {
        ...
        if stats.current_xp >= 125 {
            // Slightly higher threshold for DGM
            return true;
        }

        // Also check if agent has made enough self-modifications
        let modification_count = stats
            .xp_history
            .iter()
            .filter(|entry| {
                entry.category.contains("modification") || entry.category.contains("pattern")
            })
            .count() as u32;
...
impl DgmXPEngine {
    ...
    pub async fn initialize_dgm_xp_population(
        &self,
        size: usize,
        ...
        for individual in &population.individuals {
            ...
            agent
                .award_xp(
                    initial_xp,
                    "Initial self-modification potential".to_string(),
                    "dgm_initialization".to_string(),
                )
                .await
                .map_err(|e| EvolutionEngineError::InitializationError {
    ...
    pub async fn evolve_with_dgm_xp(
        &mut self,
    ) -> EvolutionEngineResult<(Vec<EvolutionResult>, XPEvolutionStats, DgmXPStats)> {
    ...
    async fn get_dgm_xp_stats(&self) -> EvolutionEngineResult<DgmXPStats> {
        ...
        for agent in agent_population.iter() {
            let stats = agent.stats().await;
            let modifications = stats
                .xp_history
                .iter()
                .filter(|entry| {
                    entry.category.contains("modification") || entry.category.contains("dgm")
                })
                .count() as u64;
        ...
        if let Some(agent) = agent_population.iter().find(|a| a.id() == *agent_id) {
            ...
            agent
                .award_xp(
                    total_xp,
                    format!(
                        "Self-modification: {} (improvement: {:.2})",
                        modification_type, improvement_score
                    ),
                    "self_modification".to_string(),
                )
                .await
...
impl DgmXPStats {
    pub fn summary(&self) -> String {
        format!(
            ...
            if self.self_assessment_available {
                "Active"
            } else {
                "Inactive"
            }
...
mod tests {
    ...
    async fn test_dgm_xp_fitness_function() {
        ...
        agent
            .award_xp(
                100,
                "Self-modification test".to_string(),
                "self_modification".to_string(),
            )
            .await
            .unwrap();
