...
use crate::{GlobalKnowledgeGraphResult, GlobalKnowledgeGraphError};
use crate::sync_protocol::{
    config::GlobalSyncConfig,
    types::{KnowledgeOperation, VectorClock, OperationEvidence},
    state::{KnowledgeCluster, ClusterState},
    consensus::{ConsensusEngine, PBFTConsensusEngine, ConsensusProposal},
    conflict::{ConflictResolver, DefaultConflictResolver},
    metrics::{MetricsCollector, DefaultMetricsCollector, SyncMetrics, PerformanceAnomaly, AnomalyType, AnomalySeverity, PerformanceReport},
...
pub use crate::sync_protocol::metrics::{PerformanceAnomaly, AnomalyType, AnomalySeverity, PerformanceReport};
...
impl GlobalSyncProtocol {
    ...
    pub async fn new(config: GlobalSyncConfig) -> GlobalKnowledgeGraphResult<Self> {
        ...
        let consensus_engine: Arc<dyn ConsensusEngine> = Arc::new(
            PBFTConsensusEngine::new(cluster_id.clone(), config.consensus_config.vote_threshold)
        );
    ...
    pub async fn propagate_knowledge_operation(&self, operation: KnowledgeOperation) -> GlobalKnowledgeGraphResult<()> {
        ...
        if let Some(first_version) = versions.first() {
            for (i, version) in versions.iter().enumerate() {
                if version != first_version {
                    return Err(GlobalKnowledgeGraphError::Other(
                        format!("Consistency violation: cluster {} has version {}, expected {}", 
    ...
    pub async fn get_sync_metrics(&self) -> GlobalKnowledgeGraphResult<SyncMetrics> {
        ...
        Ok(SyncMetrics {
            ...
            gpu_utilization: if self.gpu_context.is_some() { 0.85 } else { 0.0 },
    ...
    pub async fn simulate_byzantine_nodes(&self, node_indices: Vec<usize>) -> GlobalKnowledgeGraphResult<()> {
    ...
    /// Get cluster state
    pub async fn get_cluster_state(&self, region: &str) -> GlobalKnowledgeGraphResult<ClusterState> {
        for cluster in self.clusters.values() {
        ...
        Err(GlobalKnowledgeGraphError::Other(
            format!("Cluster not found for region: {}", region)
        ))
    }
    
    // Private helper methods
    
    async fn initialize_gpu_context(_config: &GlobalSyncConfig) -> GlobalKnowledgeGraphResult<Arc<GpuContext>> {
