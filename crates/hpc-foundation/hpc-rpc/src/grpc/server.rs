...
impl GrpcServerBuilder {
    ...
    pub fn with_tls(mut self, cert: CertificateWithKey) -> Result<Self> {
        ...
        if !matches!(self.state, ServerState::Server(_)) {
            return Err(RpcError::Config(
                "Cannot configure TLS after adding services. Call with_tls() before add_service()"
                    .to_string(),
            ));
        }
    ...
    fn apply_tls_config(&mut self) -> Result<()> {
        let server = match &self.state {
            ServerState::Server(_) => {
                ...
                let ServerState::Server(server) =
                    std::mem::replace(&mut self.state, ServerState::Server(Server::builder()))
                else {
                    unreachable!()
                };
                server
    ...
    pub fn build(self) -> Result<GrpcServer> {
        ...
        match self.state {
            ServerState::Router(router) => Ok(GrpcServer {
                router,
                addr: self.addr,
            }),
            ServerState::Server(_) => Err(RpcError::Config(
                "No services added to server. Call add_service() at least once before build()"
                    .to_string(),
            )),
        }
    }
