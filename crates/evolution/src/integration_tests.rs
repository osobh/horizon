...
mod tests {
    use crate::{
        AgentEvolutionEngine, AgentPerformanceFitnessFunction, LevelBasedFitnessFunction,
    ...
    async fn create_configured_test_agent(name: &str, level: u32, xp: u64) -> Agent {
        ...
        if xp > 0 {
            agent.award_xp(xp, format!("{} initial XP", name), "initialization".to_string())
                .await.unwrap();
        }
    ...
    async fn test_complete_xp_evolution_pipeline() {
        ...
        agent.update_goal_stats(true, Duration::from_secs(30)).await;
        agent.award_xp(75, "Performance bonus".to_string(), "performance".to_string()).await.unwrap();
        ...
        let evolution_results = evolution_engine.evolve_agent_population(&mut agents).await.unwrap();
    ...
    async fn test_performance_metrics_to_xp_integration() {
        ...
        for i in 0..5 {
            ...
            agent.update_goal_stats(i < 4, Duration::from_secs(execution_time)).await;
        }
        
        // Award some optimization XP to simulate performance improvements
        agent.award_xp(40, "Memory optimization".to_string(), "optimization".to_string()).await.unwrap();
        agent.award_xp(35, "Speed improvement".to_string(), "performance".to_string()).await.unwrap();
        
        let initial_xp = agent.stats().await.total_xp;
        
        // Convert performance to XP
        let performance_breakdown = performance_converter
            .convert_agent_performance_to_xp(&agent).await.unwrap();
        ...
        assert!(performance_breakdown.metric_rewards.contains_key(&PerformanceMetricType::SuccessRate));
        assert!(performance_breakdown.metric_rewards.contains_key(&PerformanceMetricType::CompletionSpeed));
    }
    ...
    async fn test_cross_agent_xp_sharing_integration() {
        ...
        coordinator.create_mentorship(&mentor_agent, &mentee_agent).await.unwrap();
        ...
        let learning_result = coordinator.apply_knowledge(&mentee_agent, &package_id).await.unwrap();
        ...
        let sharing_result = coordinator.process_xp_gain(
            &mentee_agent,
            100,
        ...
        let mentee_stats = coordinator.get_learning_stats(&mentee_agent.id()).await.unwrap();
        assert!(mentee_stats.mentor_count > 0);
        assert!(mentee_stats.knowledge_packages_learned > 0);
        
        let mentor_stats = coordinator.get_learning_stats(&mentor_agent.id()).await.unwrap();
    ...
    async fn test_multi_agent_evolution_with_collaboration() {
        ...
        coordinator.create_mentorship(&agents[4], &agents[0]).await.unwrap(); // Highest level mentors lowest
        coordinator.create_mentorship(&agents[3], &agents[1]).await.unwrap();
        
        // Simulate collaborative activities and knowledge creation
        for (i, agent) in agents.iter().enumerate() {
            // Each agent gains XP through different activities
            for j in 0..(3 + i) {
                agent.update_goal_stats(j < 2, Duration::from_secs(25 + j as u64 * 5)).await;
            }
        ...
        for (i, agent) in agents.iter().enumerate() {
            ...
            if xp_gained > 0 {
                coordinator.process_xp_gain(agent, xp_gained, "evolution_outcome").await.unwrap();
            }
        }
        
        // Verify collaborative outcomes
        assert!(evolved_count > 0, "At least some agents should have evolved");
        assert!(total_xp_gained > 500, "Total XP gain should be substantial");
        
        // Check that knowledge sharing increased overall performance
        let collective_pool = *coordinator.collective_xp_pool.read().await;
        assert!(collective_pool > 0, "Collective XP pool should have contributions");
        ...
        assert!(total_knowledge_packages >= 3, "Should have created knowledge packages");
        assert!(total_mentorships >= 4, "Should have established mentorship relationships");
    }
    ...
    async fn test_reward_calculation_consistency() {
        ...
        let evolution_breakdown = reward_calculator
            .calculate_evolution_reward(&agent, &evolution_result).await.unwrap();
        
        let fitness_improvement = 0.15; // Simulated fitness improvement
        let fitness_reward = fitness_fn.calculate_xp_reward(&evolution_result.new_metrics, fitness_improvement);
        
        let evolution_performance_xp = performance_converter
            .calculate_evolution_performance_xp(&evolution_result);
        ...
        let similar_breakdown = reward_calculator
            .calculate_evolution_reward(&agent, &similar_evolution).await.unwrap();
        
        // Rewards should be within 20% of each other for similar improvements
        let reward_diff = (evolution_breakdown.total_reward as i64 - similar_breakdown.total_reward as i64).abs();
        ...
        assert!(diff_percentage < 20.0, "Similar improvements should yield similar rewards");
    }
    ...
    async fn test_edge_cases_and_error_handling() {
        ...
        let mut agents = vec![agent.clone()];
        let evolution_results = evolution_engine.evolve_agent_population(&mut agents).await.unwrap();
        assert!(evolution_results.is_empty(), "Should not evolve agents without sufficient XP");
        ...
        assert!(learning_result.is_err(), "Should fail when learning from non-existent package");
        ...
        let mentorship_result = coordinator.create_mentorship(&low_level_mentor, &mentee).await;
        assert!(mentorship_result.is_err(), "Should fail when mentor level is too low");
        
        // Test XP sharing with zero XP
        let zero_sharing = coordinator.process_xp_gain(&agent, 0, "no_gain").await.unwrap();
        ...
        let empty_packages = coordinator.find_relevant_knowledge(
            &agent,
            &[KnowledgeType::Optimization],
            &HashSet::new(),
        ).await;
    ...
    async fn test_xp_system_scalability() {
        ...
        for i in 0..AGENT_COUNT / 4 {
            ...
            if mentor_stats.level >= 3 {
                let _ = coordinator.create_mentorship(&agents[mentor_idx], &agents[mentee_idx]).await;
            }
        }
        ...
        for agent in &agents {
            for j in 0..5 {
                agent.update_goal_stats(j < 4, Duration::from_secs(30 + j * 10)).await;
            }
        }
        ...
        assert!(evolution_time < Duration::from_secs(30), "Mass evolution should complete reasonably quickly");
        
        // Process XP sharing for all agents
        let sharing_start = std::time::Instant::now();
        for agent in &agents {
            let _ = coordinator.process_xp_gain(agent, 50, "mass_activity").await;
        }
        let sharing_time = sharing_start.elapsed();
        
        assert!(sharing_time < Duration::from_secs(10), "Mass XP sharing should be efficient");
        
        // Verify system remained consistent
        let final_collective_pool = *coordinator.collective_xp_pool.read().await;
        assert!(final_collective_pool > 0, "Collective pool should have accumulated XP");
        ...
        assert!(total_relationships > 0, "Should have maintained learning relationships");
        ...
        assert!(repository_size < 1000, "Knowledge repository should not grow unbounded");
        assert!(history_size < 10000, "Sharing history should not grow unbounded");
    }
}