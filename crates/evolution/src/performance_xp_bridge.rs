...
use stratoswarm_agent_core::agent::{Agent, AgentStats, EvolutionResult};
...
pub struct PerformanceXPConverter {
    ...
    performance_history:
        HashMap<stratoswarm_agent_core::agent::AgentId, Vec<PerformanceMeasurement>>,
...
impl Default for PerformanceXPConverter {
    fn default() -> Self {
        ...
        metric_configs.insert(
            PerformanceMetricType::CompletionSpeed,
            MetricXPConfig {
                base_reward: 30,
        ...
        metric_configs.insert(
            PerformanceMetricType::MemoryEfficiency,
            MetricXPConfig {
                base_reward: 35,
        ...
        metric_configs.insert(
            PerformanceMetricType::GpuEfficiency,
            MetricXPConfig {
                base_reward: 50,
        ...
        metric_configs.insert(
            PerformanceMetricType::ErrorRecovery,
            MetricXPConfig {
                base_reward: 45,
        ...
        metric_configs.insert(
            PerformanceMetricType::CollaborationScore,
            MetricXPConfig {
                base_reward: 55,
...
impl PerformanceXPConverter {
    ...
    pub async fn convert_agent_performance_to_xp(
        ...
        if let Some(history) = self.performance_history.get(&agent_id) {
            for measurement in history.iter().rev().take(self.evaluation_window_size) {
                ...
                breakdown
                    .metric_rewards
                    .insert(measurement.metric_type.clone(), xp_reward);
    ...
    fn generate_measurements_from_stats(&self, stats: &AgentStats) -> Vec<PerformanceMeasurement> {
        ...
        if stats.goals_processed > 0 {
            ...
            measurements.push(PerformanceMeasurement {
                ...
                statistical_confidence: if stats.goals_processed >= 10 {
                    0.8
                } else {
                    0.5
                },
        ...
        if stats.xp_history.len() >= 3 {
            let recent_xp_gains: Vec<u64> = stats
                .xp_history
                .iter()
                .take(5)
            ...
            let learning_score = if recent_xp_gains.len() >= 2 {
                ...
                let avg_gain =
                    recent_xp_gains.iter().sum::<u64>() as f64 / recent_xp_gains.len() as f64;
            ...
            measurements.push(PerformanceMeasurement {
                ...
                trend_direction: TrendDirection::Stable,
                improvement_rate: 0.0,
                statistical_confidence: if stats.xp_history.len() >= 10 {
                    0.8
                } else {
                    0.5
                },
    ...
    fn calculate_metric_xp_reward(&self, measurement: &PerformanceMeasurement) -> MetricXPReward {
        ...
        let trend_bonus = match measurement.trend_direction {
            TrendDirection::Improving => {
                (base_xp as f64 * (config.trend_bonus_multiplier - 1.0)) as u64
            }
    ...
    fn calculate_consistency_bonus(&self, agent_id: stratoswarm_agent_core::agent::AgentId) -> u64 {
        if let Some(history) = self.performance_history.get(&agent_id) {
            if history.len() >= 5 {
                ...
                let consistent_metrics = history
                    .iter()
    ...
    pub fn calculate_evolution_performance_xp(&self, evolution_result: &EvolutionResult) -> u64 {
...
impl PerformanceXPBreakdown {
    ...
    pub fn brief_summary(&self) -> String {
        ...
        format!(
            ...
            metric_count,
            if self.level_bonus > 0 {
                format!(" (+{} level bonus)", self.level_bonus)
            } else {
                String::new()
            },
            if self.consistency_bonus > 0 {
                format!(" (+{} consistency)", self.consistency_bonus)
            } else {
                String::new()
            }
        )
...
mod tests {
    ...
    async fn create_test_agent_with_performance(
        ...
        for _ in 0..goals_completed {
            agent
                .update_goal_stats(true, Duration::from_secs(execution_time_secs))
                .await;
        }

        for _ in 0..goals_failed {
            agent
                .update_goal_stats(false, Duration::from_secs(execution_time_secs + 20))
                .await;
        }
    ...
    async fn test_performance_xp_converter_creation() {
        ...
        assert!(converter
            .metric_configs
            .contains_key(&PerformanceMetricType::MemoryEfficiency));
    ...
    async fn test_performance_measurement_generation() {
        ...
        assert!(measurements
            .iter()
            .any(|m| m.metric_type == PerformanceMetricType::SuccessRate));
        assert!(measurements
            .iter()
            .any(|m| m.metric_type == PerformanceMetricType::CompletionSpeed));
    }
    ...
    async fn test_custom_metric_configuration() {
        ...
        assert_eq!(
            converter
                .metric_configs
                .get(&PerformanceMetricType::Custom(
