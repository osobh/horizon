...
mod tests {
    ...
    async fn test_marketplace_initialization() {
        let (marketplace, _sync_receiver) =
            EvolutionMarketplace::new("test_cluster".to_string(), "test_node".to_string(), 2);
    ...
    async fn test_consensus_system() {
        ...
        assert!(consensus
            .register_validator("validator_1".to_string(), vec!["sm_80".to_string()])
            .await
            .is_ok());
        assert!(consensus
            .register_validator("validator_2".to_string(), vec!["sm_80".to_string()])
            .await
            .is_ok());
    }
    ...
    async fn test_security_manager() {
        ...
        assert!(security_manager
            .add_trusted_cluster("trusted_cluster".to_string())
            .await
            .is_ok());
        ...
        // Test access check
        assert!(security_manager
            .check_access("trusted_cluster", "download")
            .await
            .unwrap_or(false));
    }
    ...
    async fn test_package_publishing_flow() {
        let (marketplace, mut sync_receiver) =
            EvolutionMarketplace::new("test_cluster".to_string(), "test_node".to_string(), 2);
        ...
        if let Ok(package_id) = result {
            ...
            let timeout_result = timeout(Duration::from_secs(1), sync_receiver.recv()).await;
