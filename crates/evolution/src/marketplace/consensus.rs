...
use crate::marketplace::types::{
    ConsensusRound, ConsensusStatus, EvolutionPackage, ResourceUsage, ValidationMetadata,
    ValidationResult, ValidatorInfo,
};
...
impl DistributedConsensus {
    ...
    pub async fn validate_algorithm(&self, package: EvolutionPackage) -> Result<bool> {
        ...
        let consensus_result = self
            .wait_for_consensus(round_id, Duration::from_secs(300))
            .await?;
    ...
    pub async fn check_consensus(&self, package_id: Uuid) -> Result<bool> {
        if let Some(validations) = self.validation_results.get(&package_id) {
            ...
            let consensus_count = trusted_validations
                .iter()
                .filter(|v| {
    ...
    pub async fn update_validator_reputation(
        ...
        if let Some(mut validator) = self.validator_registry.get_mut(cluster_id) {
            ...
            validator.reputation_score = validator.reputation_score * (1.0 - alpha)
                + (validator.reputation_score + performance_delta)
                    .max(0.0)
                    .min(5.0)
                    * alpha;
    ...
    async fn initiate_distributed_validation(&self, package: EvolutionPackage) -> Result<()> {
        ...
        for validator_id in validators {
            ...
            tokio::spawn(
                self.clone()
                    .simulate_validation_request(package.clone(), validator_id),
            );
        }
    ...
    async fn select_validators(&self, package: &EvolutionPackage) -> Result<Vec<String>> {
        ...
        let mut suitable_validators: Vec<(String, f64)> = self
            .validator_registry
            .iter()
            .filter(|entry| {
    ...
    fn validator_can_handle_package(
        &self,
    ...
    async fn filter_trusted_validations(
        &self,
