...
pub trait XPFitnessFunction: Send + Sync {
    ...
    fn evaluate_agent_fitness(&self, agent: &Agent) -> impl std::future::Future<Output = f64> + Send;
...
impl AgentPerformanceFitnessFunction {
    pub fn new(
        ...
        let total_weight = xp_weight + success_rate_weight + processing_speed_weight + memory_efficiency_weight;
...
impl XPFitnessFunction for AgentPerformanceFitnessFunction {
    async fn evaluate_agent_fitness(&self, agent: &Agent) -> f64 {
        ...
        let fitness = (normalized_xp * self.xp_weight)
    ...
    fn calculate_xp_reward(&self, fitness_improvement: f64, _evolution_metrics: &EvolutionMetrics) -> u64 {
...
mod tests {
    ...
    async fn create_test_agent_with_stats(xp: u64, goals_completed: u64, goals_failed: u64, memory_usage: usize) -> Agent {
        ...
        // Award XP
        if xp > 0 {
            agent.award_xp(xp, "Test XP".to_string(), "test".to_string()).await.unwrap();
        }
        ...
        for _ in 0..goals_failed {
            agent.update_goal_stats(false, Duration::from_secs(60)).await;
        }
    ...
    async fn test_should_evolve() {
        ...
        agent.award_xp(50, "More XP".to_string(), "test".to_string()).await.unwrap(); // Total 100 XP
        let should_evolve = fitness_fn.should_evolve(&agent).await;
        assert!(should_evolve);
    }
    ...
    async fn test_processing_speed_calculation() {
        ...
        agent2.update_goal_stats(true, Duration::from_secs(120)).await; // 120 seconds per goal
        agent2.update_goal_stats(true, Duration::from_secs(120)).await; // 120 seconds per goal
        let fitness2 = fitness_fn.evaluate_agent_fitness(&agent2).await;
