...
//!
//! Tests to verify Arc<CudaDevice> is handled correctly throughout the codebase
...
mod tests {
    ...
    /// Test that benchmark constructors handle Arc<CudaDevice> correctly
    #[test]
    fn test_benchmark_arc_device_handling() -> Result<()> {
        // Test that benchmarks expect device_id and create Arc<CudaDevice> internally
    ...
    /// Test that streaming modules handle Arc<CudaDevice> correctly
    #[test]
    fn test_streaming_arc_device_handling() -> Result<()> {
        if let Ok(device) = cudarc::driver::CudaDevice::new(0) {
            ...
            let device_arc = device; // Already Arc<CudaDevice>

            // Test GpuBufferPool accepts Arc<CudaDevice>
            let pool = crate::streaming::GpuBufferPool::new(device_arc.clone(), 2, 1024);
            assert!(pool.is_ok(), "GpuBufferPool should accept Arc<CudaDevice>");

            // Test GpuStreamProcessor accepts Arc<CudaDevice>
            ...
            assert!(
                processor.is_ok(),
                "GpuStreamProcessor should accept Arc<CudaDevice>"
            );

            // Test GpuStreamPipeline accepts Arc<CudaDevice>
            let config = crate::streaming::GpuStreamConfig::default();
            let pipeline = crate::streaming::GpuStreamPipeline::new(device_arc, config);
            assert!(
                pipeline.is_ok(),
                "GpuStreamPipeline should accept Arc<CudaDevice>"
            );
        }
    ...
    /// Test that GPU swarm handles Arc<CudaDevice> correctly
    #[test]
    fn test_gpu_swarm_arc_device_handling() -> Result<()> {
        if let Ok(device) = cudarc::driver::CudaDevice::new(0) {
            // Test that GpuSwarm stores Arc<CudaDevice> correctly
    ...
    /// Test that knowledge graph handles Arc<CudaDevice> correctly  
    #[test]
    fn test_knowledge_graph_arc_device_handling() -> Result<()> {
        if let Ok(device) = cudarc::driver::CudaDevice::new(0) {
            let device_arc = device; // Already Arc<CudaDevice>

            // Test EnhancedGpuKnowledgeGraph accepts Arc<CudaDevice>
            ...
            assert!(
                graph.is_ok(),
                "EnhancedGpuKnowledgeGraph should accept Arc<CudaDevice>"
            );

            // Test CsrGraph creation with Arc<CudaDevice>
            ...
            assert!(csr_graph.is_ok(), "CsrGraph should accept Arc<CudaDevice>");
        }
    ...
    fn test_arc_device_type_consistency() -> Result<()> {
        if let Ok(device) = cudarc::driver::CudaDevice::new(0) {
            // Verify that CudaDevice::new returns Arc<CudaDevice>
            let _arc_device: Arc<cudarc::driver::CudaDevice> = device;

            // This should compile without issues - proving the type is correct
            println!("Arc<CudaDevice> type handling is consistent");
        }
