...
use candle_nn::{activation, linear, sequential, Linear, Module, Seq, VarBuilder};
...
impl MultiGpuRoutingEngine {
    ...
    pub async fn optimize_routing(
        ...
        for (gpu_idx, chunk) in agent_pairs.chunks(pairs_per_gpu).enumerate() {
            ...
            for &(source, destination) in chunk {
                ...
                let choice = router
                    .predict_routing(source, destination, &network_state)
                    .await?;
    ...
    pub async fn rebalance_load(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !overloaded.is_empty() && !underloaded.is_empty() {
            let agents_to_move: Vec<_> = balancer
                .routing_assignments
                .iter()
                .filter(|(_, &gpu_idx)| overloaded.iter().any(|(idx, _)| *idx == gpu_idx))
    ...
    fn extract_network_features(
        ...
        features[9] = topology.connections.len() as f32
            / (topology.agents.len() * topology.agents.len()) as f32; // Connectivity
...
mod tests {
    ...
    async fn test_neural_router_initialization() {
        ...
        let router = NeuralRouter::new(device)
            .await
            .expect("Should initialize neural router");

        // Verify neural network architecture
        assert!(
            router.network.len() > 0,
            "Neural network should have layers"
        );
        assert!(
            router.routing_table.read().await.is_empty(),
            "Routing table should start empty"
        );
        assert_eq!(
            router.performance_metrics.read().await.accuracy,
            0.0,
            "Initial accuracy should be zero"
        );
    }
    ...
    async fn test_neural_routing_prediction() {
        ...
        let mut router = NeuralRouter::new(device)
            .await
            .expect("Should initialize neural router");
        ...
        assert!(
            prediction.expected_latency > 0.0,
            "Should predict positive latency"
        );
        assert!(
            prediction.expected_bandwidth > 0.0,
            "Should predict positive bandwidth"
        );
        assert!(
            prediction.reliability >= 0.0 && prediction.reliability <= 1.0,
            "Reliability should be probability"
        );
    ...
    async fn test_routing_table_learning() {
        ...
        let initial_metrics = router
            .train_network(50)
            .await
            .expect("Should train network");
        assert!(
            initial_metrics.training_epochs > 0,
            "Should complete training epochs"
        );

        // Update routing table with training data
        router
            .update_routing_table(training_examples)
            .await
        ...
        assert!(
            ...
            "Routing table should contain learned routes"
        );

        // Verify learned routes have reasonable predictions
        for (_, entry) in routing_table.iter() {
            assert!(
                entry.latency_prediction > 0.0,
                "Should predict positive latency"
            );
            assert!(
                entry.bandwidth_prediction > 0.0,
                "Should predict positive bandwidth"
            );
        ...
        for source in &agents {
            for destination in &agents {
                if source != destination {
                    ...
                    assert!(
                        route.path.len() >= 2,
                        "Path should include source and destination"
                    );
                    ...
                    assert!(
                        route.expected_latency > 0.0,
                        "Should have positive latency prediction"
                    );
                }
            }
        }
    ...
    async fn test_real_time_routing_adaptation() {
        ...
        let initial_route = router
            .predict_routing(source, destination, &initial_features)
            .await
            .expect("Should predict initial route");

        // Simulate network degradation
        let degraded_features = vec![0.1, 10.0, 0.05, 0.02, 0.9]; // Bad network conditions
        let degraded_route = router
            .predict_routing(source, destination, &degraded_features)
            .await
            .expect("Should predict degraded route");

        // Routing should adapt to changed conditions
        assert!(
            degraded_route.expected_latency > initial_route.expected_latency,
            "Should predict higher latency with degraded conditions"
        );
        assert!(
            degraded_route.expected_bandwidth < initial_route.expected_bandwidth,
            "Should predict lower bandwidth with degraded conditions"
        );
        assert!(
            degraded_route.reliability < initial_route.reliability,
        ...
        if let Some(entry) = routing_table.get(&destination) {
            assert!(
                entry.last_updated.elapsed() < Duration::from_secs(5),
    ...
    async fn test_multi_gpu_routing_engine() {
        ...
        let engine = MultiGpuRoutingEngine::new(gpu_count)
            .await
            .expect("Should initialize multi-GPU routing engine");

        assert_eq!(
            engine.gpu_contexts.len(),
            gpu_count,
            "Should have correct number of GPU contexts"
        );
        assert_eq!(
            engine.neural_routers.len(),
            gpu_count,
            "Should have neural router per GPU"
        );

        let load_balancer = engine.load_balancer.read().await;
        assert_eq!(
            load_balancer.gpu_loads.len(),
            gpu_count,
            "Should track load for each GPU"
        );
    }
    ...
    async fn test_multi_gpu_consensus_routing() {
        ...
        let routing_decisions = engine
            .optimize_routing(&topology)
            .await
            .expect("Should optimize routing with multi-GPU consensus");

        // Verify consensus produces routing decisions
        assert!(
            !routing_decisions.is_empty(),
            "Should produce routing decisions"
        );
        ...
        for ((source, dest), choice) in routing_decisions.iter() {
            assert!(
                choice.path.len() >= 2,
                "Routing path should have at least source and destination"
            );
            assert_eq!(&choice.path[0], source, "Path should start with source");
            assert_eq!(
                &choice.path.last().unwrap(),
                dest,
                "Path should end with destination"
            );
            assert!(
                choice.expected_latency > 0.0,
                "Should have positive latency"
            );
            assert!(
                choice.expected_bandwidth > 0.0,
                "Should have positive bandwidth"
            );
            assert!(
                choice.reliability >= 0.0 && choice.reliability <= 1.0,
                "Reliability should be probability"
            );
        }
    }
    ...
    async fn test_gpu_load_balancing() {
        ...
        engine
            .rebalance_load()
            .await
            .expect("Should rebalance GPU loads");

        // Verify loads are more balanced
        let load_balancer = engine.load_balancer.read().await;
        let max_load = load_balancer
            .gpu_loads
            .iter()
            .fold(0.0f32, |a, &b| a.max(b));
        let min_load = load_balancer
            .gpu_loads
            .iter()
        ...
        assert!(
            load_variance < 0.5,
            "Load balancing should reduce variance between GPUs"
        );
    }
    ...
    async fn test_neural_network_training_performance() {
        ...
        assert!(
            training_duration < Duration::from_secs(30),
            "Training should complete within 30 seconds"
        );
        assert!(metrics.accuracy > 0.7, "Should achieve >70% accuracy");
        assert!(
            metrics.training_epochs == 100,
            "Should complete requested epochs"
        );
        assert!(
        ...
        assert!(
            metrics.latency_error < 0.1,
            "Should have low latency prediction error"
        );
        assert!(
            metrics.bandwidth_error < 10.0,
            "Should have low bandwidth prediction error"
        );
    }
    ...
    async fn test_routing_performance_high_load() {
        ...
        for _ in 0..1000 {
            ...
            let task = tokio::spawn(async move {
                router_clone
                    .predict_routing(source, destination, &features)
                    .await
            });
            tasks.push(task);
        }
        ...
        assert!(
            ...
            "Should handle 1000 predictions in <10 seconds"
        );
        ...
        assert!(
            ...
            "Should successfully predict >95% of routes"
        );
    }
    ...
    async fn test_routing_failure_adaptation() {
        ...
        router
            .update_routing_table(failure_examples)
            .await
