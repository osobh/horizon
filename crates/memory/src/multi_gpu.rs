...
impl MultiGpuManager {
        ...
        let devices = {
            ...
            for i in 0..gpu_count {
                let device = CudaDevice::new(i as usize)
                    .map_err(|e| GpuMemoryError::CudaInitFailed { 
    ...
    pub fn allocate_on_gpu(&self, size: u64, gpu_id: u32) -> Result<MemoryAllocation> {
        ...
        let ptr = {
            ...
            let device_ptr = device.alloc_zeros::<u8>(size as usize)
        ...
        self.gpu_allocations.lock().unwrap()
            .get_mut(&gpu_id).unwrap()
    ...
    pub fn allocate_auto(&self, size: u64) -> Result<MemoryAllocation> {
        let gpu_id = self.load_balancer.lock().unwrap()
...
impl P2PManager {
    ...
    pub fn transfer(&self, src_gpu: u32, dst_gpu: u32, size: u64) -> Result<Duration> {
        ...
        let total_time_seconds = (transfer_stats.total_transfers as f64) * 
            (duration.as_secs_f64());
        if total_time_seconds > 0.0 {
            let gbps = (transfer_stats.total_bytes_transferred as f64) / 
                      (1024.0 * 1024.0 * 1024.0) / total_time_seconds;
...
mod tests {
    ...
    fn test_multi_gpu_allocation() {
        ...
        let alloc = manager.allocate_on_gpu(1024, 0).expect("Failed to allocate on GPU 0");
    ...
    fn test_p2p_transfer() {
        ...
        let src_alloc = manager.allocate_on_gpu(1024, 0).expect("Failed to allocate on GPU 0");
        let dst_alloc = manager.allocate_on_gpu(1024, 1).expect("Failed to allocate on GPU 1");
        
        let duration = manager.p2p_copy(&src_alloc, &dst_alloc).expect("P2P copy failed");
    ...
    fn test_gpu_utilization_tracking() {
        ...
        let _alloc = manager.allocate_on_gpu(1024 * 1024 * 1024, 0)
