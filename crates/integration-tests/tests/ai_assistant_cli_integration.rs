...
impl MockAiAssistant {
    ...
    fn process_intent(&self, intent: &str) -> Result<String, String> {
        ...
        if normalized.contains("deploy") || normalized.contains("run") {
            ...
            Ok(format!("stratoswarm deploy --image {}:latest --replicas 3", image))
            ...
            Ok(format!("stratoswarm scale {} --replicas {}", service, replicas))
    ...
    fn fuzzy_match(input: &str, pattern: &str) -> bool {
        ...
        pattern_words.iter().all(|word| {
            input_words.iter().any(|iw| iw.contains(word) || word.contains(iw))
        })
    }
    ...
    fn extract_service_name(text: &str) -> String {
        ...
        for word in &words {
            if word.contains("nginx") || word.contains("redis") || word.contains("postgres") 
                || word.contains("api") || word.contains("web") || word.contains("auth")
                || word.contains("payment") || word.contains("frontend") {
...
impl MockCliExecutor {
    ...
    fn execute(&self, command: &str) -> Result<String, String> {
        ...
        match parts.get(1).map(|s| *s) {
            Some("deploy") => {
                let image = parts.iter()
                    .position(|&p| p == "--image")
                    .and_then(|i| parts.get(i + 1))
                    .unwrap_or(&"unknown");
                let replicas = parts.iter()
            ...
            Some("status") => {
                ...
                match metric {
                    ...
                    _ => Ok("Cluster Status: Healthy\nNodes: 5\nServices: 12\nCPU: 45%\nMemory: 37.5%".to_string()),
                }
            }
...
async fn test_natural_language_deploy_command() {
    ...
    for (input, expected_service, expected_replicas) in test_cases {
        ...
        let command = ai_assistant.process_intent(input).expect("Should generate command");
        println!("Input: '{}' -> Command: '{}'", input, command);
        
        // Execute command
        let output = cli_executor.execute(&command).expect("Should execute command");
        
        // Verify output
        assert!(output.contains(expected_service), "Output should mention the service");
        assert!(output.contains(&expected_replicas.to_string()), "Output should show replica count");
...
async fn test_natural_language_query_command() {
    ...
    for (input, expected_content1, expected_content2) in test_cases {
        ...
        assert!(output.contains(expected_content1), "Output should contain: {}", expected_content1);
        assert!(output.contains(expected_content2), "Output should contain: {}", expected_content2);
    }
}
...
async fn test_complex_natural_language_understanding() {
    ...
    for input in deploy_variations {
        ...
        assert!(cmd.contains("deploy"), "Should generate deploy command for: {}", input);
        assert!(cmd.contains("nginx"), "Should identify nginx service in: {}", input);
    }
}
...
async fn test_command_execution_validation() {
    ...
    let workflows = vec![
        ...
        (
            ...
            vec!["CPU", "Memory", "Status"],
        ),
    ];
    
    for (intent, expected_outputs) in workflows {
        // Generate command from intent
        let command = ai_assistant.process_intent(intent).expect("Should generate command");
        
        // Execute command
        let output = cli_executor.execute(&command).expect("Should execute successfully");
...
async fn test_ai_learning_from_feedback() {
    ...
    let feedback = vec![
        ("deploy mysql", "stratoswarm deploy --image mysql:8.0 --replicas 1 --env MYSQL_ROOT_PASSWORD=secret"),
        ("backup database", "stratoswarm backup create --service database --type full"),
        ("restore from yesterday", "stratoswarm backup restore --timestamp yesterday --service all"),
    ];
