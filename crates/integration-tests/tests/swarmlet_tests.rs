...
mod tests {
    ...
    async fn test_swarmlet_cluster_formation() {
        ...
        assert!(results.iter().any(|r| r.phase == cluster_formation_tests::TddPhase::Red));
        assert!(results.iter().any(|r| r.phase == cluster_formation_tests::TddPhase::Green));
        assert!(results.iter().any(|r| r.phase == cluster_formation_tests::TddPhase::Refactor));
    ...
    async fn test_swarmlet_gpu_bridge() {
        ...
        for refactor_test in &refactor_gpu_tests {
            if let Some(red_test) = red_gpu_tests.iter().find(|r| r.test_name == refactor_test.test_name) {
    ...
    async fn test_enterprise_workload_simulation() {
        ...
        let has_web_app = results.iter().any(|r| 
            matches!(r.workload_type, enterprise_workload_simulation::EnterpriseWorkloadType::WebApplication { .. })
        );
        let has_database = results.iter().any(|r| 
            matches!(r.workload_type, enterprise_workload_simulation::EnterpriseWorkloadType::DatabaseCluster { .. })
        );
        let has_ml = results.iter().any(|r| 
            matches!(r.workload_type, enterprise_workload_simulation::EnterpriseWorkloadType::MachineLearning { .. })
        );
