...
pub async fn list_vendors(State(state): State<Arc<AppState>>) -> Result<Json<Vec<Vendor>>> {
    let vendors = state.repository.list_vendors().await?;
    Ok(Json(vendors))
}

pub async fn get_summary(State(state): State<Arc<AppState>>) -> Result<Json<VendorSummary>> {
