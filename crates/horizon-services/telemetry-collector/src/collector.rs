...
impl TelemetryCollector {
    ...
    pub async fn run(self) -> Result<()> {
        ...
        tracing::info!("Metrics collection enabled on port {}", self.config.observability.metrics_port);

        // Create cardinality tracker
        let cardinality_tracker = Arc::new(Mutex::new(
            CardinalityTracker::new(self.config.limits.max_cardinality)
        ));
        ...
        let listener = QuicListener::new(
            ...
            handler,
        ).await.context("Failed to create QUIC listener")?;
