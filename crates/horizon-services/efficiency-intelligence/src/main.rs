...
use efficiency_intelligence::{
    ...
    config::Config,
    db::{create_pool, EfficiencyRepository},
};
use std::sync::Arc;
...
async fn main() -> anyhow::Result<()> {
    let tracing_config = hpc_tracing::TracingConfig {
        service_name: "efficiency-intelligence".to_string(),
    ...
    info!("Starting efficiency-intelligence service");
    ...
    let repository = EfficiencyRepository::new(pool);
