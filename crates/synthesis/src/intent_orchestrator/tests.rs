...
mod tests {
    ...
    async fn test_intent_parsing() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result = orchestrator
            .parse_intent("Create a new cluster", None)
            .await;
        assert!(result.is_ok());
    ...
    async fn test_action_planning() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            steps.get(0).ok_or("Index out of bounds")?.action_type,
            execution::ActionType::Deploy
        );
    }
    ...
    async fn test_context_management() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let intent = intents::Intent {
            ...
            intent_type: intents::IntentType::Query {
                query_type: "test".to_string(),
            },
            confidence: 0.9,
            entities: vec![],
    ...
    async fn test_execution_record() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut record =
            execution::ExecutionRecord::new(uuid::Uuid::new_v4().to_string(), action_plan);
